...
public class GridRadialWorld1 implements WorldGridView, Serializable {
  ...
  Point getFwd(int dxFwd, int dyLeft) {
    ...
    return new Point(
            cr.x0 + getDir().dx * dxFwd + dirLeft.dx * dyLeft,
            cr.y0 + getDir().dy * dxFwd + dirLeft.dy * dyLeft);
  }
...
//
//    m.put("rr", new Point(0, -2));
//    m.put("frr", new Point(1, -2));
//    m.put("ffr", new Point(2, -1));

    //System.out.println("radial search begin=======");
    for( int i=0; i<=8; i++ ){
      int grad = 180 - 9 - i*20; // don't hit into straight line
      double rad = grad/180.*Math.PI;
      for( double d = 0.7; ; d += 0.7){
        // seing with different angles from center of creature front
        int fwd = 1 + (int)(d*Math.sin(rad));
        int left = (int)Math.floor(- d*Math.cos(rad));
        Point abs = getFwd(fwd, left);
        Color c = getColor(abs);
        if( !c.equals(Color.WHITE) ){
          //System.out.println(i+" "+grad+" "+d+" -> "+fwd+" "+left+ " "+getColorName(abs));
          m.put("r"+i, new Point(fwd, left));
          break;
        }
      }
    }
    //System.out.println("radial search end=======");
